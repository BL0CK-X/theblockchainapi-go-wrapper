...
func (r ApiDeleteEndpointRequest) EndpointReference(endpointReference EndpointReference) ApiDeleteEndpointRequest {
	r.endpointReference = &endpointReference
	return r
}
...
func (a *EndpointApiService) DeleteEndpointExecute(r ApiDeleteEndpointRequest) (*_nethttp.Response, error) {
	...
	// body params
	localVarPostBody = r.endpointReference
	if r.ctx != nil {
...
func (r ApiGetEndpointRequest) EndpointReference(endpointReference EndpointReference) ApiGetEndpointRequest {
	r.endpointReference = &endpointReference
	return r
}
...
/*
GetEndpoint Get an endpoint's metadata 
...
func (r ApiSetEndpointRequest) Endpoint(endpoint Endpoint) ApiSetEndpointRequest {
	r.endpoint = &endpoint
	return r
}
...
/*
SetEndpoint Create / update an endpoint 

A complete example and walkthrough of the program can be found <a href="https://github.com/BL0CK-X/api-developer-program" target="_blank">here</a>.

Create or update an `endpoint` for a `project`. 

An `endpoint` is a function that is a part of a mini-API `project`.

This endpoint adds the `endpoint` to the `project`'s documentation. However, this does not deploy the `endpoint`. To do so, you must <a href="#operation/deployProject">deploy the project</a>.

All attributes except `group_name` are required if **creating** an `endpoint`.

Only `path`, `project_id`, and `version` are required if **updating** an `endpoint`.
